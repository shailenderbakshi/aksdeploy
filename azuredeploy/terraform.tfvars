resource_group_name = "kubernetes-new-rg"
location            = "uksouth"
vnet_name           = "k8-vnet-new"
network_address_space           = ["10.0.0.0/16"]
aks_subnet_address_name        = "k8-subnet-new"
aks_subnet_address_prefix     = ["10.0.1.0/24"]
appgw_subnet_address_prefix = ["10.0.4.0/24"]
appgw_subnet_address_name    = "APG-Subnet-new"
aks_name         = "Notts-K8cluster-new"
kubernetes_version        = "1.25.6"
agent_count            = "2"
vm_size             = "Standard_B2s"
ssh_public_key           = "ssh-rsa  generated-by-azure"
log_analytics_workspace_name  = "NottsLA-new"
environment = "Prod"