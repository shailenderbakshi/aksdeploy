resource_group_name = "kubernetes-rg"
location            = "uksouth"
vnet_name           = "k8-vnet"
network_address_space           = ["11.0.0.0/16"]
aks_subnet_address_name        = "k8-subnet"
aks_subnet_address_prefix     = ["11.0.1.0/24"]
appgw_subnet_address_prefix = ["11.0.4.0/24"]
appgw_subnet_address_name    = "APG-Subnet"
aks_name         = "Notts-K8cluster"
kubernetes_version        = "1.25.6"
agent_count            = "2"
vm_size             = "Standard_B2s"
ssh_public_key           = "ssh-rsa  generated-by-azure"
log_analytics_workspace_name  = "NottsLA"
environment = "Dev"